...
interface EmployeeFormValues {
  ...
  email: string; phoneNo: string; joinDate: string
}
...
export const EmployeeCreatePage: React.FC = () => {
  ...
  const form = useForm<EmployeeFormValues>({
    initialValues: { firstName: '', lastName: '', gender: '', designationId: '', email: '', phoneNo: '', joinDate: new Date().toISOString().slice(0, 10) },
    schema: {
  ...
  const handleSubmit = async () => {
    ...
    const result = await employeeService.createEmployee({
      ...
      designationId: Number(form.values.designationId), profileImage,
    })
  ...
  return <div className="mx-auto max-w-4xl">
    ...
    <Card className="p-6">
      <FormErrorAlert message={globalError || emailServerError} title="Employee could not be created" />
      <div className="flex flex-col gap-6">
        ...
        <ProfileImageUpload file={profileImage} disabled={isSubmitting} onChange={setProfileImage} onRemove={() => setProfileImage(null)} />
      </div>
    </Card>
