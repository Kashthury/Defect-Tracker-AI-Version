...
import { Badge, Button, Card, Dropdown, ErrorMessage, Input, Loader } from '@/components/common'
...
interface FormValues { firstName: string; lastName: string; gender: Gender | ''; designationId: string; email: string; phoneNo: string; joinDate: string; active: boolean; profileImage: string }
const genders = [{ label: 'Male', value: 'MALE' }, { label: 'Female', value: 'FEMALE' }, { label: 'Other', value: 'OTHER' }]
const normalized = (v: FormValues) => ({ ...v, firstName: v.firstName.trim(), lastName: v.lastName.trim(), email: v.email.trim().toLowerCase(), phoneNo: v.phoneNo.trim(), profileImage: v.profileImage.trim() })
...
export const EmployeeEditPage: React.FC = () => {
  ...
  const form = useForm<FormValues>({
    initialValues: { firstName: '', lastName: '', gender: '', designationId: '', email: '', phoneNo: '', joinDate: '', active: true, profileImage: '' },
    schema: {
  ...
  const load = useCallback(async () => {
    ...
    if (!employeeResult.success) setLoadError(employeeResult.message)
      ...
      const item = employeeResult.data; const values: FormValues = { firstName: item.firstName, lastName: item.lastName, gender: item.gender, designationId: String(item.designationId), email: item.email, phoneNo: item.phoneNo, joinDate: item.joinDate, active: item.active, profileImage: item.profileImage ?? '' }
      setEmployee(item); setInitial(normalized(values)); form.reset(values)
    }
  ...
  const changed = useMemo(() => initial ? JSON.stringify(normalized(form.values)) !== JSON.stringify(initial) : false, [form.values, initial])
  ...
  const submit = async () => {
    ...
    const result = await employeeService.updateEmployee(id, { firstName: v.firstName, lastName: v.lastName, gender: v.gender as Gender, email: v.email, phoneNo: v.phoneNo, joinDate: v.joinDate, designationId: Number(v.designationId), active: employee.superUser ? true : v.active, profileImage: v.profileImage || null })
  ...
  return <div className="mx-auto max-w-4xl">
    ...
    <Card className="p-6">{submitError && <div className="mb-6 rounded-md bg-red-50 p-4 text-sm text-signal-critical">{submitError}</div>}<div className="flex flex-col gap-6">
      ...
      <FormRow><Input label="Join Date" type="date" required max={new Date().toISOString().slice(0, 10)} value={form.values.joinDate} error={form.touched.joinDate ? form.errors.joinDate : undefined} onChange={(e) => form.setValue('joinDate', e.target.value)} /><Input label="Profile Image URL" value={form.values.profileImage} onChange={(e) => form.setValue('profileImage', e.target.value)} /></FormRow>
