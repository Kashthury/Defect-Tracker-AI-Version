...
const backendDefectPayload = (payload: DefectPayload) => ({
  moduleId: payload.moduleId, subModuleId: payload.submoduleId, defectTypeId: payload.defectTypeId,
  severityId: payload.severityId, priorityId: payload.priorityId, releaseId: payload.releaseId,
  description: tidy(payload.description), recreationSteps: payload.recreationSteps.trim(), attachmentName: payload.attachmentName,
  assignedToId: payload.assignedToId, testCaseRequired: payload.testCaseRequired,
})
...
export const defectService = {
  ...
  async updateDefectStatus(projectId: string, id: string, toStatusId: string, changedBy = 'Super User'): Promise<ApiResponse<DefectRecord>> { void changedBy; const response = await apiRequest<BackendDefect>(`${defectEndpoint(projectId)}/${encodeURIComponent(id)}/status`, { method: 'PATCH', body: { statusId: toStatusId } }); return response.success ? ok(mapDefect(projectId, response.data), response.message) : fail(response.message) },
  async reassignDefect(projectId: string, id: string, employeeId: string, changedBy = 'Super User'): Promise<ApiResponse<DefectRecord>> { void changedBy; const response = await apiRequest<BackendDefect>(`${defectEndpoint(projectId)}/${encodeURIComponent(id)}/reassign`, { method: 'PATCH', body: { assignedToId: employeeId } }); return response.success ? ok(mapDefect(projectId, response.data), response.message) : fail(response.message) },
